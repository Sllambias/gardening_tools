    ...
    def __blur__(self, image, sigma):
        for c in range(image.shape[0]):
            if np.random.uniform() < self.p_per_channel:
                image[c] = numpy_blur(image[c], sigma, clip_to_input_range=self.clip_to_input_range)
        return image
        ...
        if not self.batched:
            ...
            for b in range(data_dict[self.data_key].shape[0]):
                for c in range(data_dict[self.data_key].shape[1]):
                    if np.random.uniform() < self.p_per_channel:
                        sigma = self.get_params(self.sigma)
                        data_dict[self.data_key][b, c] = self.__blur__(data_dict[self.data_key][b, c], sigma)
        return data_dict
